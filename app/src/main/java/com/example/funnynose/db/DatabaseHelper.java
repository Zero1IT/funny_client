package com.example.funnynose.db;

import android.content.Context;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;
import android.provider.BaseColumns;

import com.example.funnynose.network.SocketAPI;

public class DatabaseHelper extends SQLiteOpenHelper implements BaseColumns {

    private static final int DATABASE_VERSION = 1;

    private static final String DATABASE_NAME = "funnynose_cache_db";

    private static final String KEY_ID = _ID; // для всех

    // сообщения
    public static final String TABLE_CHAT = "chat_";
    public static final String KEY_MESSAGE_TEXT = "messageText";
    public static final String KEY_MESSAGE_NICKNAME = "nickname";
    public static final String KEY_MESSAGE_TIME = "messageTime";
    public static final String KEY_MESSAGE_KEY = "messageKey";


    public DatabaseHelper(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {

        for (String chatName : SocketAPI.chatNames) {
            db.execSQL("CREATE TABLE " + (TABLE_CHAT + chatName) + "(" + KEY_ID
                    + " INTEGER PRIMARY KEY AUTOINCREMENT," + KEY_MESSAGE_TEXT + " TEXT, "
                    + KEY_MESSAGE_NICKNAME + " TEXT, " + KEY_MESSAGE_TIME + " INTEGER, "
                    + KEY_MESSAGE_KEY + " INTEGER UNIQUE)");
        }

    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        for (String chatName : SocketAPI.chatNames) {
            db.execSQL("DROP TABLE IF EXISTS " + (TABLE_CHAT + chatName));
        }
        onCreate(db);
    }
}