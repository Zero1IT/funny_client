...
import by.funnynose.app.users.holders.UserHolder;
import by.funnynose.app.users.holders.UserHolderWithCity;
import by.funnynose.app.users.holders.UserHolderWithLetter;
import by.funnynose.app.users.holders.UserHolderWithParticipation;
...
public class UserListAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (mUsersFragment.getCurrentSortType() == UsersFragment.SORT_BY_CITY) {
            ((UserHolder) holder).hideCity();
        } else if (mUsersFragment.getCurrentSortType() == UsersFragment.SORT_BY_PARTICIPATION) {
    ...
    public int getItemViewType(int position) {
        if (mUsersFragment.getCurrentSortType() == UsersFragment.SORT_BY_ABC) {
        ...
        } else if (mUsersFragment.getCurrentSortType() == UsersFragment.SORT_BY_CITY) {
            if (position == 0 || !mUserList.get(position).city
                    .equals(mUserList.get(position - 1).city)) {
                return VIEW_TYPE_USER_WITH_CITY;
            }
        } else if (mUsersFragment.getCurrentSortType() == UsersFragment.SORT_BY_PARTICIPATION) {
            if (position == 0 || !Utilities.DATE_FORMAT.format(mUserList.get(position).lastParticipation)
                    .equals(Utilities.DATE_FORMAT.format(mUserList.get(position - 1).lastParticipation))) {
